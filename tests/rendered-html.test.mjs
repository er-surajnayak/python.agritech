...
test("Module 4 publishes Why Collections and exposes the complete collection roadmap", async () => {
  ...
  assert.match(moduleSource, /id: "module-4-lesson-8"[\s\S]*isPlaceholder: false/);
  assert.match(moduleSource, /id: "module-4-lesson-9"[\s\S]*isPlaceholder: false/);
  assert.equal((moduleSource.match(/id: "module-4-lesson-/g) ?? []).length, 19);
...
test("Lesson 6.5 publishes vectorized operations and broadcasting tools", async () => {
  const [moduleSource, packSource, rendererRegistrySource, rendererSource, blocksSource, stylesSource] = await Promise.all([
    readFile(new URL("content/module-6.ts", projectRoot), "utf8"),
    readFile(new URL("content/development-packs/lesson-6-5.ts", projectRoot), "utf8"),
    readFile(new URL("components/learning/LessonRenderer.tsx", projectRoot), "utf8"),
    readFile(new URL("components/learning/NumpyOperationsLessonRenderer.tsx", projectRoot), "utf8"),
    readFile(new URL("components/learning/NumpyOperationsLearningBlocks.tsx", projectRoot), "utf8"),
    readFile(new URL("src/styles/globals.scss", projectRoot), "utf8"),
  ]);

  assert.match(moduleSource, /title: "Array Operations & Broadcasting: Fast Calculations on Smart Farm Data"/);
  assert.match(moduleSource, /id: "module-6-lesson-5"[\s\S]*developmentPack: numpyOperationsDevelopmentPack/);
  assert.match(moduleSource, /id: "module-6-lesson-5"[\s\S]*isPlaceholder: false/);
  assert.match(packSource, /kind: "numpy-operations-broadcasting"/);
  for (const concept of ["temperature + 2", "temperature > 30", "(3, 4)", "a * b", "a @ b"]) assert.ok(packSource.includes(concept), `missing ${concept}`);
  assert.match(rendererRegistrySource, /lesson\.developmentPack\?\.kind === "numpy-operations-broadcasting"/);
  assert.match(rendererSource, /<ArrayCalculator/);
  assert.match(rendererSource, /<BroadcastingVisualizer/);
  assert.match(rendererSource, /<BroadcastingCompatibilityChecker/);
  assert.match(blocksSource, /export function ArrayCalculator/);
  assert.match(blocksSource, /Compare shapes from the rightmost dimension/);
...
test("Module 7.2 teaches DataFrames, inspection, columns, and real-data loading", async () => {
  const [moduleSource, packSource, registrySource, rendererSource, blocksSource, stylesSource] = await Promise.all([
    readFile(new URL("content/module-7.ts", projectRoot), "utf8"),
    readFile(new URL("content/development-packs/lesson-7-2.ts", projectRoot), "utf8"),
    readFile(new URL("components/learning/LessonRenderer.tsx", projectRoot), "utf8"),
    readFile(new URL("components/learning/PandasDataFrameLessonRenderer.tsx", projectRoot), "utf8"),
    readFile(new URL("components/learning/PandasDataFrameLearningBlocks.tsx", projectRoot), "utf8"),
    readFile(new URL("src/styles/globals.scss", projectRoot), "utf8"),
  ]);
  assert.match(moduleSource, /title: "Pandas DataFrames & Loading Real Data"/);
  assert.match(moduleSource, /id: "module-7-lesson-2"[\s\S]*developmentPack: pandasDataFrameDevelopmentPack/);
  assert.match(moduleSource, /id: "module-7-lesson-2"[\s\S]*isPlaceholder: false/);
  assert.match(packSource, /kind: "pandas-dataframes-loading"/);
  for (const concept of ["pd.DataFrame", "pd.read_csv", "pd.read_excel", "df.head", "df.info", "df.describe", "df.drop", "df.set_index", "to_csv"]) assert.ok(packSource.includes(concept), `missing ${concept}`);
  assert.match(registrySource, /lesson\.developmentPack\?\.kind === "pandas-dataframes-loading"/);
  assert.match(rendererSource, /<FarmDataFrameExplorer/);
  assert.match(rendererSource, /<CsvLoadingSimulation/);
  assert.match(rendererSource, /<DatasetLoadingWorkflow/);
  assert.match(blocksSource, /export function DataFrameCreationExplorer/);
  assert.match(blocksSource, /One labeled column/);
...
test("Module 7.4 teaches context-aware missing-data and cleaning workflows", async () => {
  const [moduleSource, packSource, registrySource, rendererSource, blocksSource, stylesSource] = await Promise.all([
    readFile(new URL("content/module-7.ts", projectRoot), "utf8"),
    readFile(new URL("content/development-packs/lesson-7-4.ts", projectRoot), "utf8"),
    readFile(new URL("components/learning/LessonRenderer.tsx", projectRoot), "utf8"),
    readFile(new URL("components/learning/PandasCleaningLessonRenderer.tsx", projectRoot), "utf8"),
    readFile(new URL("components/learning/PandasCleaningLearningBlocks.tsx", projectRoot), "utf8"),
    readFile(new URL("src/styles/globals.scss", projectRoot), "utf8"),
  ]);
  assert.match(moduleSource, /title: "Data Cleaning & Missing Data"/);
  assert.match(moduleSource, /id: "module-7-lesson-4"[\s\S]*developmentPack: pandasCleaningDevelopmentPack/);
  assert.match(moduleSource, /id: "module-7-lesson-4"[\s\S]*isPlaceholder: false/);
  assert.match(packSource, /kind: "pandas-cleaning-missing-data"/);
  for (const concept of ["isna", "isnull", "dropna", "fillna", "ffill", "bfill", "duplicated", "drop_duplicates", "astype", "to_numeric", "errors=\"coerce\"", "str.strip", "replace"]) assert.ok(packSource.includes(concept), `missing ${concept}`);
  assert.match(registrySource, /lesson\.developmentPack\?\.kind === "pandas-cleaning-missing-data"/);
  assert.match(rendererSource, /<FarmDataCleaningLab/);
  assert.match(rendererSource, /<CleaningDecisionSimulator/);
  assert.match(rendererSource, /<TypeAndTextExplorer/);
  assert.match(blocksSource, /Before/);
  assert.match(blocksSource, /Profile first, change intentionally, verify last/);
