...
const parameterOutline = [
  ["objectives", "Objectives"],
  ["story", "Story"],
  ["hardcoded-problem", "Hardcoded values"],
  ...
  ["multi-parameter", "Multiple parameters"],
...
function validateParameterLessonScope(code: string) {
  if (/\breturn\b/.test(code))
    return "Return values begin in Lesson 3.4. Display values with print() for now.";
  if (/\blambda\b/.test(code))
    return "Lambda functions are introduced later in Module 3.";
  if (/^\s*def\s+\w+\([^)]*=.+\)\s*:/m.test(code))
    return "Default parameters begin in Lesson 3.6. Use required parameters only.";
  if (/^(?!\s*def\b)\s*\w+\([^\n)]*\w+\s*=/m.test(code))
    return "Keyword arguments begin in Lesson 3.5. Supply arguments by position in this lesson.";
  return null;
}

export function FunctionParametersLessonRenderer({
  lesson,
  courseLesson,
  module,
  previous,
  ...
  if (!pack || pack.kind !== "function-parameters") return null;
  return (
    <article className="published-lesson function-parameters-development-pack">
      ...
      />
      <div className="published-lesson-layout function-parameters-lesson-layout">
        <div className="published-lesson-flow">
          <LearningObjectivesCard
            id="objectives"
            objectives={lesson.objectives}
          />
          <ParameterStory content={pack.story} />
          <HardcodedFunctionProblem content={pack.hardcoded} />
          <ParameterConceptCard content={pack.definition} />
          <ParameterizedFunctionCard content={pack.firstParameter} />
          <ParameterFlowVisualizer content={pack.parameterFlow} />
          <ArgumentParameterMapper content={pack.mapper} />
          <FunctionInputSimulator content={pack.simulator} />
          <MultiParameterExplorer content={pack.explorer} />
          <section
            id="agritech-examples"
            className="function-agritech-examples"
            aria-label="Agritech parameter examples"
          >
            {pack.agritechExamples.map((example, index) => (
              <CodeExampleCard
                key={example.title}
                id={`parameter-agritech-${index + 1}`}
                label="Agritech parameter example"
                example={example}
              />
            ))}
          </section>
          ...
          <CodePlayground
            ...
            className="function-parameters-playground"
            traceExecution
            validateCode={validateParameterLessonScope}
            renderSupplement={(_, execution) => (
              <ParameterPlaygroundSupplement execution={execution} />
            )}
          />
          <CommonMistakesCard
          ...
          <ParameterMiniProject content={pack.miniProject} />
          <ParameterComparison content={pack.comparison} />
