...
const returnOutline = [
  ...
  ["story", "Story"],
  ["print-problem", "Problem with print"],
  ...
  ["return-ends", "Return ends function"],
...
function validateReturnLessonScope(code: string) {
  if (/^(?!\s*def\b)\s*\w+\([^\n)]*\w+\s*=/m.test(code))
    return "Keyword arguments begin in Lesson 3.5. Use positional arguments in this lesson.";
  if (/^\s*def\s+\w+\([^)]*=.+\)\s*:/m.test(code))
    return "Default arguments begin in Lesson 3.6. Use required parameters only.";
  if (/\blambda\b/.test(code))
    return "Lambda functions are introduced later in Module 3.";
  if (/\braise\b|\btry\s*:|\bexcept\b/.test(code))
    return "Exception handling is outside this lesson. Focus on produced values and return flow.";
  return null;
}

export function ReturnValuesLessonRenderer({
  lesson,
  courseLesson,
  module,
  previous,
  ...
  if (!pack || pack.kind !== "return-values") return null;
  return (
    <article className="published-lesson return-values-development-pack">
      ...
      />
      <div className="published-lesson-layout return-values-lesson-layout">
        <div className="published-lesson-flow">
          <LearningObjectivesCard
            id="objectives"
            objectives={lesson.objectives}
          />
          <ReturnStory content={pack.story} />
          <PrintProblemCard content={pack.printProblem} />
          <ReturnConceptCard content={pack.definition} />
          <FirstReturnCard content={pack.firstReturn} />
          <ReturnFlowVisualizer content={pack.returnFlow} />
          <PrintReturnComparator content={pack.comparator} />
          <ValuePropagationExplorer content={pack.propagation} />
          <section
            id="agritech-examples"
            className="return-agritech-examples"
            aria-label="Agritech return examples"
          >
            {pack.agritechExamples.map((example, index) => (
              <CodeExampleCard
                key={example.title}
                id={`return-agritech-${index + 1}`}
                label="Agritech return example"
                example={example}
              />
            ))}
          </section>
          <ReturnTypesPanel content={pack.returnTypes} />
          <ReturnEndsFunction content={pack.returnEnds} />
          <IndustryInsightCard
            id="industry-insight"
          ...
          <CodePlayground
            ...
            className="return-values-playground"
            traceExecution
            validateCode={validateReturnLessonScope}
            renderSupplement={(_, execution) => (
              <ReturnPlaygroundSupplement execution={execution} />
            )}
          />
          <CommonMistakesCard
          ...
          <ReturnMiniProject content={pack.miniProject} />
          <ReturnComparison content={pack.comparison} />
